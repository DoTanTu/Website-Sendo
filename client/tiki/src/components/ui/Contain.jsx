import React from 'react'

export default function Contain() {
  return (
    <>
      <div class="main_content_right">
        <div className="top_banner_cate">
          <div className='image_main_banner'>
              <a href="">
                <picture>
                  <img src="https://salt.tikicdn.com/cache/w750/ts/tikimsp/3d/4e/c1/82b7f976c3848a354489029d62f22ac1.jpg.webp" alt="" />
                </picture>
              </a>
          </div>
          <div>
              <a href="">
                <picture>
                  <img src="https://salt.tikicdn.com/cache/w750/ts/tikimsp/3d/4e/c1/82b7f976c3848a354489029d62f22ac1.jpg.webp" alt="" />
                </picture>
              </a>
          </div>
          <div>
            <a href="">
              <picture>
                <img src="https://salt.tikicdn.com/cache/w750/ts/tikimsp/3d/4e/c1/82b7f976c3848a354489029d62f22ac1.jpg.webp" alt="" />
              </picture>
            </a>
          </div>
          <div>
            <a href="">
              <picture>
                <img src="https://salt.tikicdn.com/cache/w750/ts/tikimsp/3d/4e/c1/82b7f976c3848a354489029d62f22ac1.jpg.webp" alt="" />
              </picture>
            </a>
          </div>
          <div>
            <a href="">
              <picture>
                <img src="https://salt.tikicdn.com/cache/w750/ts/tikimsp/3d/4e/c1/82b7f976c3848a354489029d62f22ac1.jpg.webp" alt="" />
              </picture>
            </a>
          </div>
          <div>
            <a href="">
              <picture>
                <img src="https://salt.tikicdn.com/cache/w750/ts/tikimsp/3d/4e/c1/82b7f976c3848a354489029d62f22ac1.jpg.webp" alt="" />
              </picture>
            </a>
          </div>
          <div>
            <a href="">
              <picture>
                <img src="https://salt.tikicdn.com/cache/w750/ts/tikimsp/3d/4e/c1/82b7f976c3848a354489029d62f22ac1.jpg.webp" alt="" />
              </picture>
            </a>
          </div>
          <div>
            <a href="">
              <picture>
                <img src="https://salt.tikicdn.com/cache/w750/ts/tikimsp/3d/4e/c1/82b7f976c3848a354489029d62f22ac1.jpg.webp" alt="" />
              </picture>
            </a>
          </div>
          <div>
            <a href="">
              <picture>
                <img src="https://salt.tikicdn.com/cache/w750/ts/tikimsp/3d/4e/c1/82b7f976c3848a354489029d62f22ac1.jpg.webp" alt="" />
              </picture>
            </a>
          </div>
        </div>
        <div className="home_deal">
          <div className="heading_home_deal">
            <div className="left_deal">
              <span class="title">Giá Tốt Hôm Nay</span>
              <div>
                <a href="" title='Xem tất cả deal hot'>
                  <div className="count_down">
                    <span>03</span>
                    <b>:</b>
                    <span>07</span>
                    <b>:</b>
                    <span>15</span>
                  </div>
                </a>
              </div>
            </div>
            <div className="right_btn_deal">
              <a href="">Xem thêm</a>
              <img class="icon_more" src="https://salt.tikicdn.com/ts/upload/d8/38/d3/4bef23fec35f9fbfa1757d30184ddb9c.png" alt="" />
            </div>
          </div>
          <div className="body_home_deal">
            <div className="contain_list">
              <div className="btn_prev"></div>
              <div className="list_prod">
                
                <div className="item_prod">
                  <a href="">
                    <div className="image_prod">
                      <picture>
                      <img src="https://salt.tikicdn.com/cache/280x280/ts/product/6b/7d/2f/d115272bd6e17ac42bc10bb330dcbe96.jpg.webp" alt="" />
                      </picture>
                    </div>
                    <span className="deal_price_discount">-19%</span>
                    <div className="price_deal">
                      <span>237.000 đ</span>
                    </div>
                    <div className="deal_qty">
                      <div className="circle_deal"></div>
                      <span>Vừa mở bán</span>
                    </div>
                  </a>
                </div>
               
              </div>
              <div className="btn_nex"></div>

            </div>
          </div>
        </div>
        <div className="best_saler">

        </div>
      </div>
    </>
  )
}
